...
//        // GCM Instance ID의 토큰을 가져오는 작업이 시작되면 LocalBoardcast로 GENERATING 액션을
//        // 알려 ProgressBar가 동작하도록 한다.
...
//                .sendBroadcast(new Intent(QuickstartPreferences.REGISTRATION_GENERATING));
        ...
        try {
            synchronized (TAG) {
                // GCM 앱을 등록하고 획득한 설정파일인 google-services.json을 기반으로
                // SenderID를 자동으로 가져온다.
                ...
                // 토큰은 initSetting preference에 써준다.
        ...
        // GCM Instance ID에 해당하는 토큰을 획득하면 Reciever에 COMPLETE 액션을 알린다.
        // 이때 토큰을 함께 넘겨준다.
